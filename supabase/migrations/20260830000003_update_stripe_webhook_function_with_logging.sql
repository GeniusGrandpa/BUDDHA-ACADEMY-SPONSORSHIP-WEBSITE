...
BEGIN
  ...
  IF v_session_status NOT IN ('pending', 'processing') THEN
    RAISE LOG '[stripe_confirm_payment] Invalid state: session_id=%, status=%', p_session_id, v_session_status;
    RAISE EXCEPTION 'Payment session cannot be confirmed from state %', v_session_status;
  END IF;

  RAISE LOG '[stripe_confirm_payment] Creating donation: session_id=%, amount=%', p_session_id, v_amount;
  ...
  RAISE LOG '[stripe_confirm_payment] Donation created: donation_id=%, session_id=%', v_donation_id, p_session_id;
  ...
  RAISE LOG '[stripe_confirm_payment] Payment session updated: session_id=%', p_session_id;
  ...
  IF v_student_id IS NOT NULL THEN
    RAISE LOG '[stripe_confirm_payment] Creating sponsorship: student_id=%, amount=%', v_student_id, v_amount;
    ...
    IF FOUND THEN
      ...
      RAISE LOG '[stripe_confirm_payment] Student sponsorship updated: student_id=%', v_student_id;
    END IF;
  END IF;
  ...
  RAISE LOG '[stripe_confirm_payment] Function completed successfully: session_id=%, donation_id=%', p_session_id, v_donation_id;
  RETURN true;
END;
$$;
