...
export function PrivacyPageEditor() {
  ...
  const load = async () => {
    try {
      const [hdr, page] = await Promise.all([
        getPageHeader('privacy').catch(() => null),
        getPageBySlug('privacy').catch(() => null),
      ])
    ...
    } catch { toast.error('Failed to load privacy page') }
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all([
        upsertPageHeader({ page_slug: 'privacy', title: header.title, subtitle: header.subtitle, is_visible: true } as never),
        upsertPage({ slug: 'privacy', title: 'Privacy Policy', content: { body, lastUpdated } as unknown as Record<string, unknown>, published: true }),
      ])
      toast.success('Privacy page saved')
    } catch { toast.error('Failed to save privacy page') }
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Edit the privacy policy page content</p>
        </div>
